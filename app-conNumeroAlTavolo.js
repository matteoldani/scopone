  ...
  socket.on('join', function(data){
    ...
    while (!trovato && i < contatoreTavoli) {
      if(TABLE_LIST[i].id == data.num){
        trovato = true;
        TABLE_LIST[i].addPlayer(data.posizione, PLAYER_LIST[socket.id]);
        TABLE_LIST[i].numPart++;
        if(TABLE_LIST[i].numPart == 4){
          partercipanti(TABLE_LIST[i].id);
        }
      }
      console.log("ho trovato il tavolo e ha " + TABLE_LIST[i].numPart + " partecipanti");
      i++;
    }
  })
  ...
  socket2[0].on('card', function(data){
    ...
    for(var i = 0; i < mani[0].length; i++){
      if((mani[0][i].valore == data.valore) && (mani[0][i].seme == data.seme)){
        ...
        console.log("la carta giocata era la numero: ", data, "la nuova mano è: ",  mani[0]);
      }
    }

    //controllo se ho giocato un asso
    if(data.valore == 1){ //se trovo l'asso aggiugo il campo alla presa e lo svuto
    ...
    if(asso == 0){
      ...
      for(var i=0; i<campo.length;i++){
        if(campo[i].valore == data.valore){
          ...
          console.log("sto inviando le carte da togliere dal campo perchè prese");
      ...
      if(presa == 0){
          ...
          for(var i = 0; i<campo.length; i++){
            for(var j=i+1;j<campo.length;j++){
              if((campo[i].valore + campo[j].valore) == data.valore){
                ...
                prese1.push(campo[i]);
                prese1.push(campo[j]);
                console.log("ho trovato una somma e aggiungo le due carte tra quelle da toglire");
                carte.push(campo[i]);
                carte.push(campo[j]);
        ...
        if(contaSomme == 0 && sommeTriple){
          ...
          if(tipoSommaTripla == 1){
            for(var i = 0; i<campo.length; i++){
              if(campo[i].valore == 2 || campo[i].valore == 3 || campo[i].valore == 4){
                prese1.push(campo[i]);
                carte.push(campo[i]);
                prese1.push(data);
                campo.splice(i,1);
            ...
            for(var i = 0; i<campo.length; i++){
              if(campo[i].valore == 2 || campo[i].valore == 3 || campo[i].valore == 5){
                prese1.push(campo[i]);
                carte.push(campo[i]);
                prese1.push(data);
                campo.splice(i,1);
  ...
  socket2[1].on('card', function(data){
    ...
    for(var i = 0; i < mani[1].length; i++){
      if(mani[1][i].valore == data.valore && mani[1][i].seme == data.seme){
          ...
          mani[1].splice(i,1);
          console.log("la carta giocata era la numero: ", data , "la nuova mano è: " , mani[1]);
      }
    }
  ...
  socket2[2].on('card', function(data){
    ...
    for(var i = 0; i < mani[0].length; i++){
      if((mani[2][i].valore == data.valore) && (mani[2][i].seme == data.seme)){
        ...
        console.log("la carta giocata era la numero: ", data, "la nuova mano è: ",  mani[2]);
      }
    }
  ...
  socket2[3].on('card', function(data){
    ...
    for(var i = 0; i < mani[3].length; i++){
      if(mani[3][i].valore == data.valore && mani[3][i].seme == data.seme){
        ...
        console.log("dentro if per togliere carta dalla mano");
        mani[3].splice(i,1);
        console.log("la carta giocata era la numero: ", data , "la nuova mano è: " , mani[3]);
  ...
  */
  ...
  socket2[0].on('procedi', function(data){
    ...
    if(data.caso == 0){
      ...
      for(var j in data.data){
        for(var i=0; i<campo.length; i++){
          if(campo[i].valore == data.data[j].valore && campo[i].seme == data.data[j].seme){
            prese1.push(campo[i]);
            carte.push(campo[i]);
            campo.splice(i,1);
          }
  ...
  var endGame = function(){
    ...
    if(controlloZeri == 0 && controlloZeri2 == 0){
      if((totale[0]+totale[1]+totale[2]+totale[3]) > (totale2[0]+totale2[1]+totale2[2]+totale2[3])){
        punti1++;
